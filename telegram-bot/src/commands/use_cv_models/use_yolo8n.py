...
router = Router(name="Use YOLO8N")

@router.message(Command('use_yolo8n'))
async def use_yolo8n(message: Message, state: FSMContext):
    await state.set_state(UseModelStates.use_yolo8n)
    await message.answer('Отправьте изображение: ')


@router.message(UseModelStates.use_yolo8n)
async def process_image(message: Message, cv_models_service: CVModelsService, state: FSMContext):
    ...
    try:
        ...
        await cv_models_service.use_yolo8n(image=photo_bytes, chat_id=message.chat.id)
